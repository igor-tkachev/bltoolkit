...
namespace BLToolkit.Data.Linq
{
	...
	class ExpressionParser<T> : ExpressionParser
	{
			...
			ParameterExpression[] parameters)
		{
			...
			ParseInfo.CreateRoot(expression, ExpressionParam).Match(
				//
				// db.Select(() => ...)
				//
		...
		ParseInfo GetIQueriable(Expression info)
		{
			if (info.NodeType == ExpressionType.MemberAccess || info.NodeType == ExpressionType.Call)
			{
				...
				var exas = info.GetExpressionAccessors(p);
				var expr = ReplaceParameter(exas, info, _ => {});
				var l    = Expression.Lambda<Func<Expression,IQueryable>>(Expression.Convert(expr, typeof(IQueryable)), new [] { p });
				var qe   = l.Compile();
				var n    = _info.AddQueryableAccessors(info, qe);

				Expression accessor;

				ExpressionAccessors.TryGetValue(info, out accessor);
				...
				return ParseInfo.CreateRoot(qex, path);
			}
		...
		void ParseSet(LambdaInfo extract, ParseInfo update, QuerySource select)
		{
			...
			if (!ExpressionHelper.IsConstant(update.Expr.Type) && !_asParameters.Contains(update.Expr))
...
// ReSharper restore AssignNullToNotNullAttribute
			...
			var newExpr = expr.Walk(pi =>
			{
				switch (pi.NodeType)
				{
					case ExpressionType.MemberAccess:
						{
							...
							if (ex.Expr != null && ex.NodeType == ExpressionType.Constant)
							{
								...
								return pi.Parent.Replace(Expression.MakeMemberAccess(Expression.Convert(c, ex.Expr.Type), ma.Expr.Member), c);
							}
					...
					case ExpressionType.Constant:
						{
							if (ExpressionHelper.IsConstant(pi.Expr.Type))
								break;
		...
		Expression ReplaceParameter(IDictionary<Expression,Expression> expressionAccessors, Expression expr, Action<string> setName)
		{
			return expr.Convert(pi =>
			{
				if (pi.NodeType == ExpressionType.MemberAccess)
				{
					...
					if (_accessors.TryGetValue(pi, out accessor))
					{
				...
				pi.IsConstant(c =>
				{
					if (!TypeHelper.IsScalar(pi.Type) || _asParameters.Contains(c))
					{
						...
						var e = Expression.Convert(val, pi.Type);
						pi = e;
				...
				case ExpressionType.Call:
					{
						...
						else
						{
							...
							if (l != null)
							{
								var info = pi.Parent.Replace(l.Body.UnwrapLambda(), null);
								var ret  = false;

								info.Walk(lpi =>
								{
									if (!ret && IsServerSideOnly(lpi))
					...
					case ExpressionType.MemberAccess:
						{
							...
							if (ExpressionHelper.IsConstant(ma.Member.DeclaringType))
								return pi;
		...
		bool PreferServerSide(ParseInfo parseInfo)
		{
			switch (parseInfo.NodeType)
			{
				case ExpressionType.MemberAccess:
					{
						...
						if (l != null)
						{
							...
							if (l.Parameters.Count == 1 && pi.Expr.Expression != null)
							{
								info = info.Walk(wpi =>
								{
						...
						var attr = GetFunctionAttribute(pi.Expr.Member);
						return attr != null && attr.PreferServerSide && !CanBeCompiled(parseInfo);
					}
		...
		bool CanBeTranslatedToSql(LambdaInfo lambda, ParseInfo expr, bool canBeCompiled, params QuerySource[] queries)
		{
			var canbe = true;
						...
						case ExpressionType.MemberAccess:
							{
								...
								if (l != null)
								{
									canbe = CanBeTranslatedToSql(lambda, pi.Parent.Replace(l.Body.UnwrapLambda(), null), canBeCompiled, queries);
									pi.StopWalking = true;
								}
								else
								{
						...
						case ExpressionType.Call:
							{
								...
								if (e.Method.DeclaringType != typeof(Enumerable))
								{
									...
									if (cm != null)
									{
										canbe = CanBeTranslatedToSql(lambda, cm, canBeCompiled, queries);
										pi.StopWalking = true;
									}
									else
									{
