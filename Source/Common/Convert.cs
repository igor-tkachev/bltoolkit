...
namespace BLToolkit.Common
{
	public class Convert
	{
		...
		public static SqlChars ToSqlChars(object p)         
		{
			...
			if (p is Char)            return ToSqlChars((Char)p);
			if (p is TimeSpan)        return ToSqlChars((TimeSpan)p);
			if (p is DateTime)        return ToSqlChars((DateTime)p);
			#if FW3
			if (p is DateTimeOffset)  return ToSqlChars((DateTimeOffset)p);
			#endif
			if (p is Guid)            return ToSqlChars((Guid)p);

			// SqlTypes.
			//
			if (p is SqlString)       return ToSqlChars((SqlString)p);
			if (p is SqlGuid)         return ToSqlChars((SqlGuid)p);
			if (p is SqlDateTime)     return ToSqlChars((SqlDateTime)p);
