...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform">
	...
	<xsl:template name="class">
		...
		<xsl:value-of select="$lf"/>
		<xsl:value-of select="$t3"/>
		<xsl:text>if (t.IsAssignableFrom(typeof(P))) return (ConvertMethod)(object)(</xsl:text>
		<xsl:value-of select="$cls"/>
		<xsl:text>&lt;P,P&gt;.ConvertMethod)(delegate(P p) { return p; });</xsl:text>
		<xsl:value-of select="$lf"/>
		<xsl:apply-templates select="converter|comment" mode="def"/>
		<xsl:apply-templates select="converter|comment" mode="nuldef"/>
		...
		<xsl:value-of select="$lf"/>
		<xsl:apply-templates select="converter" mode="nulbody"/>
		<xsl:value-of select="$lf"/>
	...
	<xsl:template name="def">
		...
		<xsl:variable name="typepad" select="$padding - string-length(@type) - 1"/>
		<xsl:value-of select="$lf"/>
		<xsl:value-of select="$t3"/>
	...
	<xsl:template match="converter" mode="nuldef">
	...
	<xsl:template name="body">
		...
		<xsl:text>,P&gt;.ConvertMethod)(delegate(P p)</xsl:text>
	...
	<xsl:template match="from">
		...
		<xsl:text>&gt;.ConvertMethod)(delegate(</xsl:text>
		<xsl:value-of select="@type"/>
		<xsl:call-template name ="writeSpaces">
			<xsl:with-param name="count" select="$typepad"/>
		</xsl:call-template>
